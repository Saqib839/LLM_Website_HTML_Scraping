...
def ask_llm_for_extraction_groq_openai(api_key, cleaned_text, website_url):
    """
    Use Groq Python client to extract structured doctor information from cleaned text.
    ...
    try:
        with open("demo_prompt.txt", "r", encoding="utf-8") as f:
            demo_prompt = f.read()
    except Exception:
        demo_prompt = (
    ...
    try:
        ...
        if parsed is None:
            try:
                ...
                print(f"Groq API did not return valid JSON for {website_url}")
                return None
        ...
        for entry in parsed:
            if not isinstance(entry, dict):
                continue
            person = {k: str(entry.get(k, "")).strip() for k in expected_keys}
        ...
        print(f"Groq API extraction failed for {website_url}: {e}")
        return None


def ask_llm_for_extraction_qwen(api_key, cleaned_text, website_url):
    """
    Use Groq Python client with qwen/qwen3-32b model (streaming) 
    to extract structured doctor information from cleaned text.
    ...
    try:
        ...
        for chunk in completion:
            ...
            print(delta, end="")  # optional: print live streaming
            text += delta
...
def main():
    ...
    parser.add_argument("--model", default=None, help="Model name from Hugging Face Hub")
    ...
    for url in urls:
        ...
        people = ask_llm_for_extraction_groq_openai(API_KEY, cleaned_text, url)
